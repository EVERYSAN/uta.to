...
function timeJP(d: string | Date) {
  try { return new Date(d).toLocaleString("ja-JP", { timeZone: "Asia/Tokyo" }); } catch { return ""; }
}
  ...
  return (
    <main style={{ maxWidth: 1100, margin: "20px auto", padding: 16 }}>
      ...
      <form style={{ display: "flex", gap: 8, marginBottom: 16 }}>
        <input name="q" defaultValue={q} placeholder="キーワード" style={{ flex: 1, padding: 10, border: "1px solid #ddd", borderRadius: 8 }} />
        ...
        <button type="submit" style={{ padding: "10px 14px", borderRadius: 8, background: "black", color: "white" }}>検索</button>
      </form>
        ...
        {items.map((v: any) => (
          <Link key={v.id} href={`/video/${v.id}`} style={{ textDecoration: "none", color: "inherit" }}>
            <article style={{ border: "1px solid #eee", borderRadius: 10, overflow: "hidden" }}>
              {v.thumbnailUrl && <img src={v.thumbnailUrl} alt={v.title} style={{ width: "100%", aspectRatio: "16/9", objectFit: "cover" }} />}
              <div style={{ padding: 10 }}>
                <div style={{ fontWeight: 600, fontSize: 14, marginBottom: 6, lineHeight: 1.4 }}>{v.title || "No title"}</div>
                <div style={{ fontSize: 12, color: "#666" }}>{v.creator?.name ?? "Unknown"}・{v.publishedAt ? timeJP(v.publishedAt) : ""}</div>
