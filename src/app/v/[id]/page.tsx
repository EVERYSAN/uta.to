...
const fmtDate = (iso?: string | null) => {// src/app/v/[id]/page.tsx
import { PrismaClient } from "@prisma/client";
import { notFound } from "next/navigation";
import Link from "next/link";
import ClientActions from "./ClientBits";

export const dynamic = "force-dynamic"; // キャッシュ無効（最新表示）
...
function TrendingBadge() {
  return (
    <div className="inline-flex items-center gap-1 rounded-full bg-violet-600/20 text-violet-300 px-2 py-0.5 text-[11px]">
      <span>⬆</span><span className="font-medium">急上昇</span>
...
export default async function VideoDetailPage({ params }: Params) {
  ...
  // 1) DBのidで検索 → 2) 見つからなければ platformVideoId でも検索
  let v =
